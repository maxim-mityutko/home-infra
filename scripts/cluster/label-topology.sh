...
#
# Purpose:
#   Helper for labeling Kubernetes nodes with topology zones.
#
# What this script does:
#   1) Lists all nodes in the current Kubernetes cluster.
#   2) Asks which nodes should be assigned to which topology zones.
#   3) Labels each selected node with:
#      - topology.kubernetes.io/zone=<selected value>
#
# Notes:
#   - Run from a shell with kubectl access to the cluster.
#   - Labels are applied with --overwrite.
...
prompt_nodes() {
  ...
  while true; do
    ...
    read -r -p "Select nodes to label (numbers/names, comma or space separated; 'none' to skip): " input
    input="${input//,/ }"
...
apply_labels() {
  ...
  log "Applying topology label to node ${node}..."
